...
// Manually define line
// svg.append('line')
//     .attr('x1', point1[0]).attr('y1', point1[1] + 20)
//     .attr('x2', point2[0]).attr('y2', point2[1] + 20)
//     .style('stroke', 'black');

const g = svg.append('g')
    .attr('id', 'line1');

// Will not have desired effect
// const lines = g.selectAll('line').data([point1, point2]).join('line');

const lines = d3.select('svg #line1')
    .selectAll('line')
    .data([linePoints])
    .join('line')
...
const modifiedPoints = d3.pairs(points);
...
svg.append('g')
    ...
    .data([data2])
    .join('path')
    .attr('d', d3.line().x(d => d[0]).y(d => d[1] + 20))
    .style('stroke', 'black')
    .style('fill', 'none')
